...
def compute_spo_outcome_advantage(
    ...
    **kwargs,
    ...
    """SPO (Segment Policy Optimization) outcome advantage.

    Group-normalize scalar rewards (GRPO-style), then distribute each response's
    normalized reward across `n_segs` segments with linearly increasing weights
    (later tokens get more credit). Optionally zero out advantages on tokens with
    probability above `prob_mask_threshold` (requires old_log_probs).
    """
    cutpoint_interval = 5
