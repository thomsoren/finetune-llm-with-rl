#!/usr/bin/env bash
# Row 1: GRPO + AdamW  (baseline)
# 4x RTX 3090 24GB, FSDP, vLLM rollout
set -uo pipefail
...
/venv/main/bin/python -m rl_finetuning.train \
    ...
    trainer.experiment_name=row1_grpo_adamw_gsm8k \
    ...
    trainer.default_local_dir=/workspace/rl-finetuning/checkpoints/row1 \
    2>&1 | tee /workspace/rl-finetuning/logs/row1.log

ray stop --force 2>/dev/null || true
echo "Row 1 done: $(date)"
